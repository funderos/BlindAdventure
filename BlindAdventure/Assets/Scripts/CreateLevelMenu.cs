...
public class CreateLevelMenu : MonoBehaviour
{
    ...
    public Button startRecordingObstacleButton; //Button to start the voice recording 
    public Button stopRecordingObstacleButton; //Button to stop the voice recording
    private DateTime dateTimeStart1; //Timestamp 1
    private DateTime dateTimeStart2; //Timestamp 2
    private List<KeyValuePair<int, Obstacle>> obstacleList; //List with the obstacles of a steeplechase
    ...
    public void onNewNodeButtonClick()
    {
        if (swiped == false)
        {
            ...
            { //If the changed node isn't saved
                Handheld.Vibrate();
    ...
    public void onChangeNodeButtonClick()
    {
        if (swiped == false)
        {
            ...
            { //Deletes the recorded questions
    ...
    public void onSaveButtonClick()
    {
        ...
        {
            Handheld.Vibrate();
            if (saveNodeButton == true)
            { //If the node isn't saved
                if (changeNode == false)
                { //If its a new node, the node will be placed at the end of the list. Otherwise the node will be changed.
                    nodeList.Add(node);
                }
                level.setList(nodeList);
                LoadSaveGame.saveLevel(level);
    ...
    //Starts with the recording of a new obstacle
    public void onStartRecordingObstacleButtonClick()
    {
        if (swiped == false)
        {
            Handheld.Vibrate();
    ...
    //Stops the recording of an obstacle and save it
    public void onStopRecordingObstacleButtonClick()
    {
        if (swiped == false)
        {
            Handheld.Vibrate();
    ...
    IEnumerator directionObstacle()
    {
        ...
        obstacleList.Add(new KeyValuePair<int, Obstacle>(steepleChase.getObstacleNumber(), obstacle));
