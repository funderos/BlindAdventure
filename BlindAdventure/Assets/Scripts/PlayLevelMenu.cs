...
public class PlayLevelMenu : MonoBehaviour
{
    ...
    public void playNode()
    {
        ...
        else
        {
            ...
            if (node.getQuiz() != null || node.getFight() != null || node.getSteeplechase() != null)
            { //If its a node with a minigame
            ...
            { //Only a node with an item -> collect it
                playNodeButton.interactable = false;
    ...
    IEnumerator finishedLevel()
    {
        ...
        if (!File.Exists(Application.persistentDataPath + "/CurrentGame/Level_" + nextLevelNumber + ".txt"))
        { //If its the last level -> output: game completed
    ...
    IEnumerator fightOpponent(int index)
    {
        ...
        { //opponent defeated
            ...
            { //Fight won
                ...
                if (node.getNodeSolved() == false)
                { //Node solved for the first time
                    while (TTSManager.IsSpeaking())
                    {
                        yield return null;
                    }
    ...
    IEnumerator waitToObstacle(int secondsToWait)
    {
        ...
        TTSManager.Speak(xmlReader.translate("PlayLevelMenuObstacleIntro"), false);
        if (File.Exists(Application.persistentDataPath + "/CurrentGame/Level" + levelNumber + "NodeNumber" + nodeIndex + "Obstacle" + obstacleNumber + ".wav"))
        {
            ...
            UnityWebRequest wav = UnityWebRequestMultimedia.GetAudioClip(
                "file://" + Application.persistentDataPath + "/CurrentGame/Level" + levelNumber + "NodeNumber" + nodeIndex + "Obstacle" + obstacleNumber + ".wav", AudioType.WAV);
            yield return wav.SendWebRequest();
            if (wav.isNetworkError)
            {
                Debug.Log(wav.isNetworkError);
    ...
    IEnumerator avoidObstacle(int directionObstacle)
    {
        ...
        if (direction == directionObstacle || (directionObstacle == 2 && direction == 0))
        { //Steeplechase lost -> Restart?
            restartMinigameButton.interactable = true;
        ...
        { //Obstacle evaded
            ...
            if (obstacleIndex < obstacleList.Count)
            { //Next obstacle
                StartCoroutine(waitToObstacle(obstacleList[obstacleIndex].Value.getTime()));
            }
            else
    ...
    IEnumerator question()
    {
        if (File.Exists(Application.persistentDataPath + "/CurrentGame/Level" + levelNumber + "NodeNumber" + nodeIndex + "Question" + currentQuestion + ".wav"))
        {
            ...
            UnityWebRequest wav = UnityWebRequestMultimedia.GetAudioClip(
                "file://" + Application.persistentDataPath + "/CurrentGame/Level" + levelNumber + "NodeNumber" + nodeIndex + "Question" + currentQuestion + ".wav", AudioType.WAV);
            yield return wav.SendWebRequest();
            if (wav.isNetworkError)
            {
                Debug.Log(wav.isNetworkError);
    ...
    IEnumerator solutionQuestion()
    {
        ...
        { //Quiz not successfully completed -> Restart?
            restartMinigameButton.interactable = true;
    ...
    public void nextNode(int direction)
    {
        ...
        if (direction == 11)
        { //Decrease the nodeIndex if isn't zero
            if (nodeIndex != 0)
            {
                nodeIndex--;
                StartCoroutine(walkToNode());
    ...
    public IEnumerator itemOutput()
    {
        ...
        {
            UnityWebRequest wav = UnityWebRequestMultimedia.GetAudioClip(
                "file://" + Application.persistentDataPath + "/CurrentGame/Level" + levelNumber + "NodeNumber" + nodeIndex + "Item" + ".wav", AudioType.WAV);
